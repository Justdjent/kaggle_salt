...
def main():
    ...
    def make_loader(file_names, shuffle=False, transform=None, mode='train', problem_type='binary'):
        return DataLoader(
            dataset=SaltDataset(file_names, transform=transform, problem_type=problem_type, mode=mode),
            shuffle=shuffle,
    ...
    train_images = [img + '.png' for img in train_df['id'].values]
    val_images = [img + '.png' for img in val_df['id'].values]
    ...
    valid_loader = make_loader(val_images, transform=val_transform, mode='valid', problem_type=args.type)
